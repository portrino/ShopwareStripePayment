/**
 * A common utility object for handling payments using Stripe.js in the ViisonStripePayment plugin.
 */
var ViisonStripePayment = {
	...
	handleStripeError: function(message) {
		...
		$('#viison-stripe-error-box').css('display', 'block').children('.error-content').html(message);
		$('body').animate({
	...
	setupCVCPopupControl: function() {
		var cvcInfoPopup = $('#viison-stripe-cvc-info-popup');
		if (this.isShopware5Template()) {
			...
			$('#viison-stripe-cvc-info-popup-close').click(function(event) {
				...
				$.overlay.close();
				cvcInfoPopup.hide();
				$('#viison-stripe-form').append(cvcInfoPopup);
			});
		} else {
			// Shopware 4
			$('#viison-stripe-cvc-info-button').click(function(event) {
	...
	observeFormSubmission: function() {
		...
		this.findForm().on('submit', function(event) {
			...
			if (!me.isViisonStripePaymentSelected()) {
	 ...
	 */
	isViisonStripePaymentSelected: function() {
